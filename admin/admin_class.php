...
Class Action {
	...
	function save_apartment(){
		...
		$data .= ", price = '$price' ";
		$chk = $this->db->query("SELECT * FROM apartments where apartment_no = '$apartment_no' ")->num_rows;
		if($chk > 0 ){
			return 2;
			exit;
		}
	...
	function save_tenant(){
	    ...
	    if(empty($id)){
	        // Insert new tenant
	        $save_user = $this->db->query("INSERT INTO users SET $data");
	    }else{
	        // $save_user = $this->db->query("UPDATE users SET $data WHERE id = $id");
	        $save_tenantapartment = $this->db->query("UPDATE tenantapartment SET apartment_id = '$apartment_id', date_in = '$date_in' WHERE tenant_id = $id");
	        if($save_user && $save_tenantapartment) 
	            return 1;
	    }
	...
	function save_tenante(){
	    ...
	    if(empty($id)){
	        ...
	        $save = $this->db->query("UPDATE users SET $data WHERE id = $id");
	        // $save = $this->db->query("UPDATE tenantapartment SET $data WHERE tenant_id = $id");
	    }
	    if($save) 
	        return 1;
	}
	...
	function get_tdetails(){
		extract($_POST);
		$data =array();
		$tenants =$this->db->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.apartment_no,h.price FROM tenants t inner join apartments h on h.id = t.apartment_id where t.id = {$id} ");
